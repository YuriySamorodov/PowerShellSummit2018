...
Configuration TestConfig {
    ...
    node $AllNodes.Where({$_.Role -eq 'DC'}).NodeName {
        WindowsFeature DomainController{
            Name= $ConfigurationData.NonNodeData.Feature
...
TestConfig -OutputPath .\ -ConfigurationData .\5.Non-Node.psd1 # Running the Config and produce mofs in the current directory