...
Configuration TestConfig {
    ...
    node $AllNodes.Where({$_.Role -eq 'DNS'}).NodeName {
        $DNSData = $ConfigurationData.DNSData
        WindowsFeature DNS{
            Name= $DNSDATA.Feature
            Ensure= $Node.EnsurePresent
        }
    }
}

TestConfig -OutputPath .\ -ConfigurationData .\7.BetterNon-Node.psd1 # Running the Config and produce mofs in the current directory