...
Configuration TestConfig {
    $LabData = Import-PowerShellDataFile -Path .\8.EvenBetter.psd1
   ...
   # node $AllNodes.Where({$_.Role -eq 'DC'}).NodeName {
    ...
    $ConfigurationData.AllNodes.Nodename{
        $ConfigurationData.Roles.($Node.Role).configurations.Foreach{
            ...
            WindowsFeature $_ {
                Name= $ConfigurationData.Roles.$ConfigName.Feature
...
TestConfig -OutputPath .\ -ConfigurationData .\8.EvenBetter.psd1 # Running the Config and produce mofs in the current directory